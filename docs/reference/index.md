...
The recommended lifecycle is `feature setup (branch/context alignment) -> discover -> constitution -> specify -> design when needed -> plan -> tasks -> analyze -> implement -> test -> review -> verify -> release`. `brief` remains legacy artifact language rather than a recommended default stage.
