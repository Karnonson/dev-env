...
- The full SDD cycle is: **Strategist → speckit.constitution → speckit.specify → Designer / speckit.design → speckit.plan → speckit.tasks → speckit.analyze → Backend Dev / UI Builder → speckit.test → Code Reviewer → kite verify feature**.
- Use Strategist to explore and pressure-test ideas first. After approval, route through `speckit.constitution` to lock project standards before specification.
